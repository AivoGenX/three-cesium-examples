    ...
    constructor(viewer, options) {
        ...
        if (this.initOptions?.points) {
            ...
            for (let i in this.initOptions.points) {
                ...
                const x = ((point.x - bounds[0]) / (bounds[2] - bounds[0])) * width; //灞忓箷鍧愭爣x
                const y = ((bounds[3] - point.y) / (bounds[3] - bounds[1])) * height; //灞忓箷鍧愭爣y
                const dataPoint = {
                    x: x,
                    y: y,
                    value: point.value,
...
const viewer = new Cesium.Viewer(DOM, {
    ...
    animation: false,//鏄惁鍒涘缓鍔ㄧ敾灏忓櫒浠讹紝宸︿笅瑙掍华琛�    

    baseLayerPicker: false,//鏄惁鏄剧ず鍥惧眰閫夋嫨鍣紝鍙充笂瑙掑浘灞傞€夋嫨鎸夐挳

    baseLayer: Cesium.ImageryLayer.fromProviderAsync(Cesium.ArcGisMapServerImageryProvider.fromUrl('https://server.arcgisonline.com/arcgis/rest/services/World_Imagery/MapServer')),

    fullscreenButton: false,//鏄惁鏄剧ず鍏ㄥ睆鎸夐挳锛屽彸涓嬭鍏ㄥ睆閫夋嫨鎸夐挳

    timeline: false,//鏄惁鏄剧ず鏃堕棿杞�    

    infoBox: false,//鏄惁鏄剧ず淇℃伅妗�   
